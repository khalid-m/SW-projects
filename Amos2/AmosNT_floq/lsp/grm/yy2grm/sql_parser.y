 ...
 * $Revision: 1.1 $ $Date: 2011/10/16 16:41:58 $
 * $State: Exp $ $Locker:  $
 *
...
%{
   ...
   oidtype oidrefs = nil;     /* List of objects created during parsing */
   #define SAVE_REF a_setf(oidrefs,cons(yyval.oidval,oidrefs))
   #define reverse(x)(call_lisp(mksymbol("nreverse"),varstack,1,x))
   #define nconc2(L1, L2)(call_lisp(mksymbol("nconc2"),varstack,2,L1,L2))
...
%token DELETE DISTINCT DOUBLE DROP ELSE END ESCAPE EXISTS _FALSE FLOAT
%token FOREIGN FROM GROUP HAVING IN INDICATOR INNER INSERT INTEGER INTO
%token IS JOIN KEY LIKE MAX MIN NATRUAL NOT _NULL NUMERIC OID ON ORDER
...
%token USER VALUES VARCHAR VARYING WHEN WHERE WORK
...
sql:                    select_statement opt_semicolon //';'
                 {a_setf(sql_statement,$<oidval>1); a_free(oidrefs); lineno=1;}
                        |
                        create_table_statement opt_semicolon //';'
                 {a_setf(sql_statement,$<oidval>1); a_free(oidrefs); lineno=1;}
                        |
                 ...
                 {a_setf(sql_statement,$<oidval>1); a_free(oidrefs); lineno=1;}
                        |
                        commit_statement opt_semicolon //';'
                 {a_setf(sql_statement,$<oidval>1); a_free(oidrefs); lineno=1;}
                        |
                        rollback_statement opt_semicolon //';'
                 {a_setf(sql_statement,$<oidval>1); a_free(oidrefs); lineno=1;}
                        |
                        drop_statement opt_semicolon //';'
                 {a_setf(sql_statement,$<oidval>1); a_free(oidrefs); lineno=1;}
                        |
                        set_schema_statement opt_semicolon //';'
                 {a_setf(sql_statement,$<oidval>1); a_free(oidrefs); lineno=1;}
                        |
                        error opt_semicolon //';'
                 {a_free(sql_statement); a_free(oidrefs); lineno=1;}
  			;
...
drop_statement:         DROP TABLE table opt_drop_type
                 {$<oidval>$ = a_list(mksymbol("SQL-DROP-TABLE"),
                                      $<oidval>3, NULL); SAVE_REF;}
                        ;

opt_drop_type:   {$<oidval>$ = nil;}
                        |
                        RESTRICT
                        |
                        CASCADE
...
rollback_statement:     ROLLBACK opt_work_keyword
                 {$<oidval>$ = cons($<oidval>1, nil); SAVE_REF;}
;
...
update_statement:       UPDATE table SET update_column_commalist opt_where_clause
                 {$<oidval>$ = a_list(mksymbol("SQL-UPDATE"), $<oidval>2,
                                      $<oidval>3, reverse($<oidval>4), NULL);
                  if($<oidval>5 != nil)
                    $<oidval>$ = nconc2($<oidval>$,
...
update_column_commalist:
                 ...
                 {$<oidval>$ = cons($<oidval>3, $<oidval>1); SAVE_REF;
                   /* Reverse */}
                        ;
...
update_column:   {$<oidval>$ = nil;}
                        |
                        NAME EQ scalar_exp
                 {$<oidval>$ = cons($<oidval>1, $<oidval>3); SAVE_REF;}
                        ;
...
delete_statement:       DELETE FROM table opt_where_clause
                 {$<oidval>$ = a_list(mksymbol("SQL-DELETE"), $<oidval>3, NULL);
                  if($<oidval>4 != nil)
                    $<oidval>$ = nconc2($<oidval>$, cons($<oidval>4, nil));
                  SAVE_REF;}
                        ;
...
/*                      a_list(mksymbol("SQL-INSERT"), $<oidval>3,
                                      $<oidval>4, $<oidval>5, NULL);
                  $<oidval>$ = nconc2($<oidval>$, $<oidval>6);
*/                  SAVE_REF;}
                        ;

opt_columns:     {$<oidval>$ = NULL;}
                        |
                        '(' column_commalist ')'
                 {$<oidval>$ = reverse($<oidval>2); SAVE_REF;}
                        ;
...
values_commalist:       '(' scalar_exp_commalist ')'
                 {$<oidval>$ = cons(reverse($<oidval>2), nil); SAVE_REF;}
                        |
                        values_commalist ',' '(' scalar_exp_commalist ')'
                 {$<oidval>$ = nconc2($<oidval>1, cons(reverse($<oidval>4), nil));
                  SAVE_REF;} 
                        ;
...
data_type:              type0
                 ...
                 {$<oidval>$ = cons($<oidval>1, $<oidval>2); SAVE_REF;}
                        |
...
opt_type_param1: {$<oidval>$ = nil;}
                        |
                        '(' INTNUM ')'
                 {$<oidval>$ = cons($<oidval>2, nil); SAVE_REF;}
;

opt_type_param2:        opt_type_param1
                        |
                        '(' INTNUM ',' INTNUM ')'
                 {$<oidval>$ = a_list($<oidval>2, $<oidval>4, NULL); SAVE_REF;}
                        ;
...
type1:                  FLOAT
                        | CHARACTER
                        | VARCHAR
                        | CHARACTER VARYING { $<oidval>$ = mksymbol("VARCHAR"); SAVE_REF;}
                        | CLOB
                        ;
...
opt_default_spec:
                 ...
                 {$<oidval>$ = a_list($<oidval>1, $<oidval>2, NULL); SAVE_REF;}
                        ;
...
col_constraints_list:
			...
			|	col_constraints_list col_constraint
                 {$<oidval>$ = nconc2($<oidval>1,$<oidval>2); SAVE_REF;}
  			;
...
simple_col_constraint:  NOT _NULL
                 {$<oidval>$ = cons(mksymbol("NOT-NULL"), nil); SAVE_REF;}
                        |
                        PRIMARY KEY
                 {$<oidval>$ = cons(mksymbol("PRIMARY-KEY"), nil); SAVE_REF;}
                        |
                        UNIQUE
                 {$<oidval>$ = cons($<oidval>1,nil); SAVE_REF;}
                        |
                 ...
                 {$<oidval>$ = a_list($<oidval>1, $<oidval>3, NULL); SAVE_REF;}
                        ;

table_constraint:       CONSTRAINT NAME simple_table_constraint
                 {$<oidval>$ = nconc2(a_list($<oidval>1,
                                             $<oidval>2, NULL),
                                      $<oidval>3); SAVE_REF;}
                        |
...
simple_table_constraint:
                 ...
                 {$<oidval>$ = a_list(mksymbol("FOREIGN-KEY"), $<oidval>4,
                                      $<oidval>6, $<oidval>7, $<oidval>8, NULL);
                  SAVE_REF;}
                        ;
...
table_ref_commalist:	table_ref
                  {$<oidval>$ = cons($<oidval>1,nil);SAVE_REF;}
			|	table_ref_commalist ',' table_ref
                  {$<oidval>$ = cons($<oidval>3,$<oidval>1);SAVE_REF;}/* Reverse */
			|	table_ref_commalist INNER JOIN table_ref ON search_condition
                  {$<oidval>$ = cons(a_list(mksymbol("INNERJOIN"),
                                            call_lisp(mksymbol("SECOND"), varstack, 1, $<oidval>4),
                                            call_lisp(mksymbol("THIRD"), varstack, 1, $<oidval>4),
                                            $<oidval>6, NULL),
                                     $<oidval>1);SAVE_REF;}/* Reverse */
			|	table_ref_commalist JOIN table_ref ON search_condition
                  {$<oidval>$ = cons(a_list(mksymbol("INNERJOIN"),
                                            call_lisp(mksymbol("SECOND"), varstack, 1, $<oidval>3),
                                            call_lisp(mksymbol("THIRD"), varstack, 1, $<oidval>3),
                                            $<oidval>5, NULL),
                                     $<oidval>1);SAVE_REF;}/* Reverse */
...
table_ref:		table
                  {$<oidval>$ = a_list(nil, $<oidval>1, NULL); SAVE_REF;}
			|	table range_variable
                  {$<oidval>$ = a_list(nil,$<oidval>1,$<oidval>2, NULL); SAVE_REF;}
			|	table AS range_variable
                  {$<oidval>$ = a_list(nil,$<oidval>1,$<oidval>3, NULL); SAVE_REF;}
			;
...
opt_group_by_clause:	/* empty */
			...
			|  GROUP BY column_ref_commalist
                  {$<oidval>$ = reverse($<oidval>3); SAVE_REF;}
			;
...
order_ref_commalist:	scalar_exp order_dir
                  {$<oidval>$ = cons(a_list($<oidval>2, $<oidval>1, NULL),nil); SAVE_REF;}
			|	order_ref_commalist ',' scalar_exp order_dir
                  {$<oidval>$ = cons(a_list($<oidval>4, $<oidval>3, NULL),$<oidval>1); SAVE_REF;
		    /* Reverse */}
			;

order_dir:              /* empty */
                  {$<oidval>$ = mksymbol("ASC");SAVE_REF;}
                        | ASC
                  {$<oidval>$ = $<oidval>1;}
                        | DESC
                  {$<oidval>$ = $<oidval>1;}
...
selection:      projection
         ...
         |      relation_to_stream '(' projection ')'
                  {$<oidval>$ = a_list($<oidval>3, $<oidval>1, NULL); SAVE_REF;}
;
...
function_ref:		AMMSC '(' scalar_exp ')'
                  {$<oidval>$ = a_list($<oidval>1,$<oidval>3,NULL); SAVE_REF;}
                        |
                        COUNT '(' scalar_exp ')'
                  {$<oidval>$ = a_list(mksymbol("SQL_COUNT"),$<oidval>3,NULL); SAVE_REF;}
                        |
                        COUNT '(' '*' ')'
                  {$<oidval>$ = a_list(mksymbol("SQL_COUNT"),$<oidval>3,NULL); SAVE_REF;}
                        |
                        COUNT '(' ALL scalar_exp ')'
                  {$<oidval>$ = a_list(mksymbol("SQL_COUNT"),$<oidval>4,NULL); SAVE_REF;}
                        |
                        COUNT '(' DISTINCT scalar_exp ')'
                  {$<oidval>$ = a_list(mksymbol("SQL_COUNTD"),$<oidval>4,NULL); SAVE_REF;}
                        |
                        MIN '(' opt_all_distinct scalar_exp ')'
                  {$<oidval>$ = a_list(mksymbol("SQL_MIN"),$<oidval>4,NULL); SAVE_REF;}
                        |
                        MAX '(' opt_all_distinct scalar_exp ')'
                  {$<oidval>$ = a_list(mksymbol("SQL_MAX"),$<oidval>4,NULL); SAVE_REF;}
                        |
                        AVG '(' scalar_exp ')'
                  {$<oidval>$ = a_list(mksymbol("SQL_AVG"),$<oidval>3,NULL); SAVE_REF;}
                        |
                        AVG '(' ALL scalar_exp ')'
                  {$<oidval>$ = a_list(mksymbol("SQL_AVG"),$<oidval>4,NULL); SAVE_REF;}
                        |
                        AVG '(' DISTINCT scalar_exp ')'
                  {$<oidval>$ = a_list(mksymbol("SQL_AVGD"),$<oidval>4,NULL); SAVE_REF;}
                        |
                        SUM '(' scalar_exp ')'
                  {$<oidval>$ = a_list(mksymbol("SQL_SUM"),$<oidval>3,NULL); SAVE_REF;}
                        |
                        SUM '(' ALL scalar_exp ')'
                  {$<oidval>$ = a_list(mksymbol("SQL_SUM"),$<oidval>4,NULL); SAVE_REF;}
                        |
                        SUM '(' DISTINCT scalar_exp ')'
                  {$<oidval>$ = a_list(mksymbol("SQL_SUMD"),$<oidval>4,NULL); SAVE_REF;}
			;
...
window:         rows_window
            ...
            |   RANGE INTNUM time_unit
                  {$<oidval>$ = a_list($<oidval>1, $<oidval>2, $<oidval>3, NULL); SAVE_REF;}                  
            |   PARTITION BY column_ref_commalist rows_window
                  {$<oidval>$ = a_list($<oidval>1, $<oidval>3, $<oidval>4, NULL); SAVE_REF;}
            ;
