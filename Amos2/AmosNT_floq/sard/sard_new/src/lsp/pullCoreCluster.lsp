...
;;; $RCSfile: pullCoreCluster.lsp,v $
;;; $Revision: 1.2 $ $Date: 2012/04/25 20:24:20 $
;;; $State: Exp $ $Locker:  $
;;;
;;; Description: Functions for puliing the common cluster fn out of a predicate
;;;;;; ===========================================================================
...
(defun back_comp_costpred (fno)
 ...
 (defun compute-exec-cost-pred (optpreds bnd)
  (catch 'predcost;;An exception can be raised in andpredcost
    (let* ((disjs (if (eq 'or (car optpreds)) (cdr optpreds) nil));If optperds is an OR
	   (ands (cond ((eq (car optpreds) 'and) (cdr optpreds)); ;If optperds is an AND
		       ((atom optpreds) nil)
		       (t (list optpreds))))
	   (cost (list 0 0)))
      (if (null disjs) 
	  (setq cost (andpredcost ands bnd))
        ...
        ;;Cost of a disjunction is the sum of the cost of its elements (of the disjuncts)
	(dolist (branch disjs)    
