...
http://user.it.uu.se/~udbl/software/WampServer2.0a.exe
	...
	sample database 'example_db' with username 'u1' and password
	'12345'.
...
8. Reloading the Archived Schema and Column-Order Data files(Sorted by Foreign keys).
		...
		loadSQLColORderInsert(:a,'NTArchives\prod1.nt','http://user.it.uu.se/~udbl/sard/prod');


9. Reloading the Archived Schema and Column-Order Data files(No Foreign keys).
	
		loadSQLNoCheckColORderInsert(:a,'NTArchives\prod1.nt','http://user.it.uu.se/~udbl/sard/prod');

		
10. Reloading the Archived Schema and Row Order Archive Data (No BaseURI).
		
		loadSQLRowInsertNoURI(:a,'NTArchives\prod1.nt');


11. Reloading the Archived Schema and Column Order Archive Data Files (No BaseURI).
		
		loadSQLColInsertNoURI(:a,'NTArchives\prod1.nt');


12. Clean realtional database (if required).
	
		cleanDatabase(:a,'http://user.it.uu.se/~udbl/sard/prod');
		cleanDatabase(:a);


13. Clean imported schema table (if required).
