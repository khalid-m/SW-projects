...
;;; $Revision: 1.13 $ $Date: 2014/03/05 16:51:32 $
;;; $State: Exp $ $Locker:  $
;;;
;;; Description: Rewrites for unbloun-propertyfor queries to and RDF view over RDBMS
;;;;;; ===========================================================================
...
(defun subsublist (l ll)
  "Subtsract the list l from list ll"
  (let ((res nil)
	(llels nil))
...
"Rewrites a conjunction whose conjuncts include rowid-uri and ustr_literal and FILTERs "
  (cond ((atom con) con)
	((null (cddr con)) con)
	((null (eq 'AND (car con))) con);;if con is not a conjunction
	(t
	 (let ((ht (make-hash-table :test (function equal)))
	       (res con))	  
	   (dolist (c (cdr con) )
	     (if (neq 'OPTIONAL (first c) );;OPTIONAL not rewritten
              (progn
	       (if (or (eq (oid-name (first c)) 'CHARSTRING.CHARSTRING.LIKE->BOOLEAN);; if c is a like or comparison predicate    
		      (eq (oid-name (first c)) 'LITERAL.USTR_STR->CHARSTRING)
		     (eq (oid-name (first c)) 'OBJECT.OBJECT.>->BOOLEAN);; if c is a >
		     (eq (oid-name (first c)) 'OBJECT.OBJECT.>=->BOOLEAN);
		     (eq (oid-name (first c)) 'OBJECT.OBJECT.<->BOOLEAN) ;; if c is a <
		     (eq (oid-name (first c)) 'OBJECT.OBJECT.<=->BOOLEAN) ;; if c is a <=
		     (eq (oid-name (first c)) 'OBJECT.OBJECT.!=->BOOLEAN);; if c is != 
		     (eq (oid-name (first c)) 'NUMBER.NUMBER.PLUS->NUMBER);; if c is plus
		     (eq (oid-name (first c)) 'NUMBER.NUMBER.TIMES->NUMBER);; if c is plus
		     (eq (oid-name (first c)) 'LITERAL.LITERAL.RDF:PLUS->LITERAL)
		     (eq (oid-name (first c)) 'VECTOR.IN->OBJECT)
		     (eq (oid-name (first c)) 'NUMBER.NUMBER.GREATER->NUMBER)
		     (eq (oid-name (first c)) 'NUMBER.NUMBER.LESSER->NUMBER)
		     (eq (oid-name (first c)) 'NUMBER.ABS->NUMBER)
		     (eq (oid-name (first c)) 'NUMBER.RDF:ABS->NUMBER)
		     (eq (oid-name (first c)) 'VECTOR.IN->OBJECT)
		     (eq (oid-name (first c)) 'OBJECT.OBJECT.=->BOOLEAN))
		 (progn 
		   (let ((e nil) (diff nil))
		     (if (eq (oid-name (first c)) 'VECTOR.IN->OBJECT);;hardcoded
			 (setf diff (cddr c))
		       (setf diff (cdr c)))
		     (mapcar (f/l (e)
				  (if (gethash e ht);;if there is an elem in ht with key=var
				      (setf (gethash e ht) (cons c (gethash e ht)))
				    (setf (gethash e ht) (list c))) ) diff) )    ))
		;; if c is either rowid_uri or ustr_literal predicate
		(if (or (eq (oid-name (first c)) 'CHARSTRING.VECTOR.ROWID_URI->URI)
			(eq (oid-name (first c)) 'CHARSTRING.USTR->USTR)
			(eq (oid-name (first c)) 'LITERAL.USTR_LITERAL->LITERAL) )		
		      (if (gethash (car (last c)) ht) ;;if there is an elem in ht with key=last var
			 (setf (gethash (car (last c)) ht) (cons c (gethash (car (last c)) ht))) ;;if yes, adds to by 'cons'
			(setf (gethash (car (last c)) ht) (list c)))) ) ) );; no puts the 'like' pred in ht with key =subject		
	   ;;(help)
	   (maphash (f/l (key predlist);;goes through the ht			
			 (let ((rowid nil) 
			       (likelist nil)
			   ...
			   (dolist (pr predlist rowid)			     	
			     (if (cdr predlist)
				(progn
				  (if (or (eq (oid-name (first pr)) 'CHARSTRING.VECTOR.ROWID_URI->URI)
					  (eq (oid-name (first pr)) 'CHARSTRING.USTR->USTR)
					  ;;(eq (oid-name (first pr)) 'LITERAL.USTR_STR->CHARSTRING)
					  (eq (oid-name (first pr)) 'OBJECT.OBJECT.=->BOOLEAN)
					  (eq (oid-name (first pr)) 'LITERAL.USTR_LITERAL->LITERAL))
				       (setf rowid pr)))))
			   (if (null (null rowid));;if rowid is not empty
			       (progn			
				(setf likelist (subsublist predlist rowid));;the rest of predlist (without rowid)			
				(mapcar (f/l (els)  ;;goes through the rest of the predlist				    
					     (cond ((and (or (eq (oid-name (first rowid)) 'LITERAL.USTR_LITERAL->LITERAL)
							     (eq (oid-name (first rowid)) 'CHARSTRING.USTR->USTR))
							     (neq (oid-name (first els)) 'CHARSTRING.VECTOR.ROWID_URI->URI)
							     (neq (oid-name (first els)) 'LITERAL.USTR_STR->CHARSTRING))
						    (setf newel (cons (first els) (subst (second rowid) key (cdr els))));;replaces one bound var by a generated variable  
						    (if (eq (typename (caddr els)) 'ustr)
							(setf newel (subst (ustr+- (caddr els)) (caddr els) newel)))
						    (let ((e nil) (di nil) (elsss nil))
						      (if (eq (oid-name (first els)) 'VECTOR.IN->OBJECT);;hardcoded
							(progn
							 
							     (setf di (arraytolist (car (set-difference (cdr els)(list key)))))
							     (mapcar (f/l (e)									
								       (setf (gethash e ht) (subst newel els (gethash e ht))) ;replaces the pred els in the hash table       
								       (setf elsss (cons (ustr+- e) elsss))) ;substitutes ustr with the str					   
								         di)
							   (setf newel (subst (listtoarray elsss) (second newel) newel))
							   (setf res (andify (list newel res)))  )
						         (progn
							 (setf di (set-difference (cdr els)(list key)))						 
							 (mapcar (f/l (e)
								 (if (gethash e ht);;if it exists another key=another var in diff
								     (progn									  
								       (setf (gethash e ht) (subst newel els (gethash e ht))) ;replaces els in the hash table
								       (setf res (subst NIL els res))) ) ) di)))) ;;eliminates els from res
						    ;;(help)
						      (setf res (andify (list newel (subsublist res els)))));;contsructs res
						    ((and (eq (oid-name (first rowid)) 'CHARSTRING.USTR->USTR);;needed for SPARQL rdf:regex
							  (eq (oid-name (first els)) 'LITERAL.USTR_STR->CHARSTRING))
							      (setf newel (cons  _=_ (subst (second rowid) key (cdr els))));var=var1 where ustr(var) in view defs equal and ustr_str(var1) from regex
						     (setf res (andify (list newel (subsublist res els) ))) );;contsructs res
						    ((and (eq _is_Literal_ t);;activate the rewrite is_literal
						           (eq (oid-name (first rowid)) 'CHARSTRING.VECTOR.ROWID_URI->URI)
							   (or (eq (oid-name (first els)) 'CHARSTRING.CHARSTRING.LIKE->BOOLEAN)
							       (eq (oid-name (first els)) 'OBJECT.OBJECT.>->BOOLEAN)
							       (eq (oid-name (first els)) 'OBJECT.OBJECT.<->BOOLEAN)))
						     (setf res nil));;rewrite the conjunction to nil if (AND rowid like)	
						     ((and (eq _is_Literal_ t);deactivate the rewrite is_literal
						      ...
						      (mapcar (f/l (e)
								 (if (gethash e ht);;if it exists another key=another var in diff
								     (progn									  
								       (setf (gethash e ht) (subst newel els (gethash e ht))) ;replaces the pred els in the hash table
								       (setf res (subst NIL els res))) ) ) di)   ) ;;eliminates els from res 			   
						     ;;(setf res (andify (list (subsublist res els) newel))) );;contsructs res
						     (setf res (andify (list newel (subsublist res els)))) );;contsructs res
						    ((and (eq (oid-name (first rowid)) 'CHARSTRING.USTR->USTR);;needed for SPARQL rdf:regex
							  (eq (oid-name (first els)) 'LITERAL.USTR_STR->CHARSTRING))
						      (setf newel (cons  _=_ (subst (second rowid) key (cdr els))));var=var1 where ustr(var) in view defs equal and ustr_str(var1) from regex
...
(defun rewrite-filter-recurs (con)
"Rewrites a conjunction whose conjuncts include rowid-uri and ustr_literal and FILTERs 
 First try to make the function recursive "
  (cond ((atom con) con)
	((null (cddr con)) con)
	;;((null (eq 'AND (car con))) con);;if con is not a conjunction
	(t
	 (let ((ht (make-hash-table :test (function equal)))
	       (res con))	  
	   (dolist (c (cdr con) )
	     (if (listp (cadr c));;OPTIONAL or other embedded clause	
		  (progn (help)  (setf res (andify (list res (rewrite-filter (cdr c))))))
	       (progn
	         (if (or (eq (oid-name (first c)) 'CHARSTRING.CHARSTRING.LIKE->BOOLEAN);; if c is a like or comparison predicate    
		      (eq (oid-name (first c)) 'LITERAL.USTR_STR->CHARSTRING)
		     (eq (oid-name (first c)) 'OBJECT.OBJECT.>->BOOLEAN);; if c is a >
