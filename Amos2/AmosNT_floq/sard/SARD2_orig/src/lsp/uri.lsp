...
;;; $RCSfile: uri.lsp,v $
;;; $Revision: 1.2 $ $Date: 2013/11/01 15:03:48 $
;;; $State: Exp $ $Locker:  $
;;;
...
(defun commonels (fno l1 l2)
  "Finds whether l1 and l2 have common elements"
  (let ((res nil)
	(l (arraytolist l1))
	(ll (arraytolist l2)))
    (setf res nil)
    (dolist (x l) 
      (dolist (y ll)
       (if (equal x y) (setf res t))   ))
   (osql-result l1 l2 res)))
