...
;;; $Revision: 1.9 $ $Date: 2013/10/28 11:09:26 $
;;; $State: Exp $ $Locker:  $
;;;
			   ...
			   (if (null (null rowid));;if rowid is not empty
				...
				(mapcar (f/l (els)  ;;goes through the rest of the predlist				    
					     (cond ((and (or (eq (oid-name (first rowid)) 'LITERAL.USTR_LITERAL->LITERAL)
						    ...
						    (setf newel (cons (first els) (subst (second rowid) key (cdr els))));;replaces one bound var by generated
						     ;;(help)
						    (let ((e nil) (di nil))
...
(defun rewrite-filter2 (con)
"Rewrites a conjunction whose conjuncts include rowid-uri and ustr_literal and FILTER "
  (cond ((atom con) con)
	((null (cddr con)) con)
	((null (eq 'AND (car con))) con);;if con is not a conjunction
	(t
	 (let ((ht (make-hash-table :test (function equal)))
	       (res con))	  
	   (dolist (c (cdr con) )
	      (if (eq (oid-name (first c)) 'LITERAL.LITERAL.COMPARABLE->BOOLEAN)
		  (setf res (subst NIL c res)))
	     (if (or (eq (oid-name (first c)) 'CHARSTRING.CHARSTRING.LIKE->BOOLEAN);; if c is a like or comparison predicate
		     (eq (oid-name (first c)) 'OBJECT.OBJECT.>->BOOLEAN);; if c is a >
		     (eq (oid-name (first c)) 'OBJECT.OBJECT.<->BOOLEAN) ;; if c is a <
		     (eq (oid-name (first c)) 'OBJECT.OBJECT.!=->BOOLEAN);; if c is != 
		 ...
		 (progn 
		   (let ((e nil) (diff nil))
		     (if (eq (oid-name (first c)) 'VECTOR.IN->OBJECT)
			 (setf diff (cddr c))
		       (setf diff (cdr c)))		   
		     (mapcar (f/l (e)
				  (if (gethash e ht);;if there is an elem in ht with key=bounded var
				      (setf (gethash e ht) (cons c (gethash e ht)))
				    (setf (gethash e ht) (list c)))) diff)   )		   
		   ))
		;; if c is either rowid_uri or ustr_literal predicate
	   ...
	   (maphash (f/l (key predlist);;goes through the ht			
			 (let ((rowid nil)
			   ...
			   (if (null (null rowid));;if rowid is not empty
				 ...
				 (setf likelist (subsublist predlist rowid));;the rest of predlist (without rowid)
				 (mapcar (f/l (els)  ;;goes through the rest of the predlist				    
					     (cond ((and (eq (oid-name (first rowid)) 'LITERAL.USTR_LITERAL->LITERAL)
							  (neq (oid-name (first els)) 'CHARSTRING.VECTOR.ROWID_URI->URI) )
						     (progn 	  
						      (if (eq (oid-name (first els) 'LITERAL.LITERAL.RDF:PLUS->LITERAL))
							       (setf newel (cons (getfunctionnamed 'NUMBER.NUMBER.PLUS->NUMBER) (subst (second rowid) key (cdr els))))
							     (setf newel (cons (first els) (subst (second rowid) key (cdr els)))));;replaces one bound var by generated
							     
							   (if (symbolp (car (set-difference (cdr newel) (ret-genvar (cdr newel)))))
							       (if (gethash (car (set-difference (cdr newel) (ret-genvar (cdr newel)))) ht);;if it exists another key=bounded var      
								   (progn
								     ;;(help)
								     (setf (gethash (car (set-difference (cdr newel) (ret-genvar (cdr newel)))) ht) 
									   (subst newel els (gethash (car (set-difference (cdr newel) (ret-genvar (cdr newel)))) ht )));;replaces the pred els in the hash table
								     (setf res (subst NIL els res)))));;eliminates els from res 
							   (setf res (andify (list (subsublist res els) newel))) ;;contsructs res 
 (help)						
							   ))
						    ((and (eq _is_Literal_ t);;activate the rewrite is_literal
