...
(defun absorb-gql (this rest)
  ...
  (let* ((ccfno (car this))
    ...
    (dolist (var ccVarList);;absorb non-symbol in 'this' if there is any
      (cond ((not (symbolp var));;
	     (dolist (val (gethash var ht))
	       (tconc absorbedList (list '= val var)))))) ;(help 1)
    (cond (rest
	   (dolist (pred rest)
    ...
    (setq filterPred (list* (theresolvent filterFuncName)  
			    generatedExpression ccVarList)) ;(help 2)
    (cond (lastRun 
...
(defglobal _hj_ (getfunctionnamed 'BAG.BAG.NUMBER.NUMBER.FASTHASHJOIN->VECTOR))
(defglobal _gql2_ (getfunctionnamed 'CHARSTRING.CHARSTRING.gql->VECTOR))
...
(defun translate-gql (tr bnd)
  ...
  (let* ((filterFno (car tr))
    ...
    (setq packedParamList (list* _vector-dynconstructor_ paramVector 
				 parameters))
    (setq gqlCallList (list _gql_ dsn queryString paramVector 
			    queryResult))
...
(defun build-condition_1 (filter ht)
  "transform pred to gql filter format in where clause"
  (let ((res ""))
    (mapc (f/l (pred) 
	       (let ((val (gethash (second pred) ht)))
		 (cond (val  ;(help)
			(cond ((stringp (third pred))
			      (setq res (concat res (gethash (second pred) ht) 
			      (car pred) "'" (third pred) "'"  " and "))
			       )
			      ((numberp (third pred))
			(setq res (concat res (gethash (second pred) ht) 
					  (car pred) (third pred) " and ")))))
...
(defun construct-whereClause_1 (filter ht)
  (cond (filter
	 (concat "where " (build-condition_1 filter ht)))
	(t "")))
(quote
(defun translate-gql (tr bnd)
  "TR rewrite rule that generates a new TR predicate to call GQL 
  ...
  (let* ((filterFno (car tr))
    ...
    (mapc (f/l (var col)(setf (gethash var ht) col))
	  varlist collist)
    (setq whereClause (construct-whereClause_1 gqlFilter ht)) ;(help 1)
    (quote
    (cond (bnd
           (setq whereClause (construct-whereClause gqlFilter ht)) 
	   (setq queryString (concat "select * from " table " " whereClause 
...
(setq gqlCallList (list _gql_ dsn queryString queryResult)) ;(help)
;(setq unpackedResultList (list* _vector-dynconstructor_ queryResult 
;				    varList))
;(setq conjunctionList (list gqlCallList unpackedResultList))
(tconc queryCallList gqlCallList)
(cond (bnd   ;(help)
(setq hjCall (list _hj_ (car queryCallList) 1))
    (if unabsorbedPreds (setq conjunctionList 
			      (append conjunctionList unabsorbedPreds)))
    (andify conjunctionList))
(t (andify '(()())))
...
(quote
(defun translate-gql (trList bnd)
  "TR rewrite rule that generates a new TR predicate to call GQL 
    for a given FILTER call in TR format"
  (let ((queryCallList (tconc))
	(unabsorbedPredsList (tconc))
	(unpackedResultList2 (tconc))
	hjCall
	(ResultVec (dt_genvar _vector_))
	unpackedResultList
	)
    (dolist (tr trList)
      (cond ((listp tr)
	     (let* ((filterFno (car tr))
	       ...
	       (setq unpackedResultList (append unpackedResultList varList))
	       (setq gqlCallList (list _gql2_ dsn queryString queryResult)) ;(help)

;(setq conjunctionList (list gqlCallList unpackedResultList))
	       (tconc queryCallList gqlCallList)

	       (if unabsorbedPreds (tconc unabsorbedPredsList unabsorbedPreds))))))

    (setq hjCall (list _hj_ (caar queryCallList) (cadr queryCallList) 1 0 ResultVec))
    (setq unpackedResultList2 (list* _vector-dynconstructor_ ResultVec 
				     unpackedResultList)) ;(help 2)
    (help)
    (cond ((car unabsorbedPredsList)
	   (andify (list hjCall unpackedResultList2 (car unabsorbedPredsList))))
	  (t
	   (andify (list hjCall unpackedResultList2))
	   ))
    ))
)
