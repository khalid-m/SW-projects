 ...
 * <ul>
 *   <li> makes it easier to generate html-code for tables and lists from a amos2-scan </li>
...
/**
 * resultToTable creates a html-string representing the content of a scan as a table.<br>
 * if the scan contains a vector, the vector will be layouted as a table inside a cell of the parent-table, with the same formating
 * <br><br>
 *
 * @param scan the result scan that has to be output as a html-table
 * @param boolean oneTableColumnPerVector specifies if vectors should be layouted as one row or as one column.<br>
 ...
 *                <ul>tableProperties is "bordercolor='#000000' border='1' cellspacing='0' cellpadding='5'" if not set.</ul>
 *
 * @return string representing the content of the scan as a html-table
 *
 */
  public String resultToTable(Scan scan, String tableProperties) throws callin.AmosException {
    // apply standard-table-layout if no layout passed.
    if (tableProperties.equalsIgnoreCase("")){
    ...
    while (!scan.eos() ){
      resultString = resultString + tupleToTableRow((Tuple) scan.getRow(),false,false,tableProperties, "");
      scan.nextRow();
  ...
  *
  * @return String html-table-row(s) for the tuple with sub-tables if the tuple contains a mix of vectors and non-vectors
  */
  private String tupleToTableRow(Tuple tuple, boolean oneTableColumnPerVector, boolean startNewTable,  String tableProperties, String spacer) throws callin.AmosException {
    String resultString = "";
    ...
    while (column < tuple.getArity()) { // for each column in each tuple
      ...
      if(tuple.isTuple(column)){ // column contains a tuple
        ...
        resultString = resultString + tupleToTableRow(subTuple, !oneTableColumnPerVector, newTable, tableProperties, spacer+"      ");
...
/**
 * resultToTable(Scan scan) takes a scan and creates a html-string according to the supplied pattern for every tuple
 *                          inside the scan.<br><br>
 * @param Scan theScan
 * @param String pattern    html-pattern to be used to create output with the scans tuples<br>
 *                          <ul>
 *                          e.g.  pattern = "&lt;tr&gt;\n &lt;td&gt;\n &lt;a href='&amp;col_1;'target='_blank'&gt;\nLink to &amp;col_1;\n &lt;/a&gt;\n &lt;/td&gt;\n&lt;/tr&gt;\n";
 *                          </ul>
 *
 * @return String created according to the supplied pattern for every tuple inside the scan.
 *
 */

  public String resultToLayout(Vector resultVec, String pattern){
    Tuple scanRow = null;
    String resultString = "";
    String msg = "";
    if (resultVec.size() == 0 ) {
      msg = "The Query you submitted, returned no Scan. Pobably the Amos-Object you called doesn't exist!";
  ...
  public String resultToLayout(Scan theScan, String pattern){
    Tuple scanRow = null;
    String resultString = "";
    String msg = "";
    if (theScan == null) {
  ...
  private String tupleToLayout(Tuple tuple, String pattern){
    String resultString = "";
    String msg = "";
    String element = "";
    int tplBeginTag = 0;
  ...
  /////////// if the tuple doesn't contain a vector (SeqElem) create create html-string with this tuple
        ...
        while (pattern.substring(tplBeginTag).indexOf("&col_") > -1){
          ...
          try {
            ...
            msg = "Exception!! : The Tag '" + pattern.substring(tplBeginTag+5, tplEndTag) + "' contains an invalid number-format \n" + e;
            System.out.println(msg);
            break;
          }
          try{
            ...
            msg = "Exception!! : The column '" + tplToShow + "' does probably not exist in the tuple! " + e;
            System.out.println(msg);
          }
 ...
 */
  ...
  public Hashtable getRequestValues(ServletRequest request, String[] values){
    String[] val = values;
    Hashtable valuesHashTable = getRequestValues(request);
    int i = 0;
    while ( i < val.length){
      if(!valuesHashTable.containsKey(val[i])){
        valuesHashTable.put(val[i],"");
      }
      i++;
    }
 ...
 */
  public Hashtable getRequestValues(ServletRequest _request){
 ...
 * <ul>
 *   <li>email : it will be checked if it could be a valid email, containing at least one "." and exactly one @ at the proper positions</li>
 ...
 */
  ...
  public boolean validate(String _input, String _pattern, int _maxlength, boolean _notNull, int _minlength){
    boolean result = true;
    ...
    if ( pattern.startsWith("email"))  {
      ...
      //   @ exists     // only one @ // . after @ exists // the last dot is max 3rd but last character   //  none of the 36 checked bad characters exists
      if ( indexOfAt < 1 | ats != -1  | !(lastDot > 1)  |  !(lastDot < behindTheAt.length()-2) | !(badChar==-36) ) {
        result = false;
      }
    }
   ///////////////////////////////////////////////////////////
  ...
  public boolean validate(String _input, String _pattern){
    return validate(_input, _pattern, -1,  false, -1);
  }
  public boolean validate(String _input, String _pattern, boolean _notNull){
    return validate(_input, _pattern,  -1,  _notNull, -1);
  }
  public boolean validate(String _input, String _pattern, int _maxlenght, boolean _notNull){
    return validate(_input, _pattern,  _maxlenght,  _notNull, -1);
  }
  public boolean validate(String _input, String _pattern, int _maxlenght, int _minlength){
    return validate(_input, _pattern,  _maxlenght,  false, _minlength);
  }
  public boolean validate(String _input, String _pattern, int  _minlength){
 ...
 */
  public Object notNull(Object object, String returnClass){
 ...
 */
  ...
  public String toSingleQuote(String s){
...
/**
 * Method to replace any occuence of a string in a string
 * <br><br>
 *
 ...
 */
  ...
  public String replace(String text, String removeString, String replaceString) {
    int idx = text.indexOf(removeString);
    while(idx != -1) {
      text = text.substring(0, idx) + replaceString + text.substring(idx + removeString.length());
