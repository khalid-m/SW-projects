...
void pipemergebbff(a_callcontext cxt, a_tuple params) {
  ...
  for (child_id=0; child_id<2; child_id++) {
    a_setf(uppipe[child_id], new_pipestream(cxt->env));
    a_setf(downpipe[child_id], new_pipestream(cxt->env));
    ...
    if(forkstate < 0) {
      ...
      {
	/* Tore's advice: Pad risky sections
	   w/ unwind_protect_{begin|catch (is ALWAYS executed)|end} */
	unwind_protect_begin;
	mapbag(cxt->env, bag[child_id], pipemerge_mapper, (void*)&state);
...
void register_pipemerge(void) {
  a_extfunction("MERGEBBFF",pipemergebbff);
}
