...
WSRequest.prototype.send = function(payload) {
    ...
    if (accessibleDomain) {
	   ...
	   //this._xmlhttp.open(method, 'http://udbl2.it.uu.se:8082/wsmos/service/AmosServlet', this._async,this._username,this._password);
    ...
    _buildSOAPEnvelope : function(soapVer, options, url, content, username, password) {
        ...
        var request = '<?xml version="1.0" encoding="UTF-8"?>\n' +
                  '<s:Envelope xmlns:s="' + ns + '"' +
                   wsaNsDecl + ' xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" '+ '>\n' +
                  '<s:Header>' + headers + '</s:Header>\n' +
                  '<s:Body s:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/" >' + (content != null ? content : '') + '</s:Body>\n' +
                  '</s:Envelope>';
        return request;
