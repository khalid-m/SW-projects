 ...
 * $RCSfile: xsearch.c,v $
 * $Revision: 1.1 $ $Date: 2012/01/04 14:48:50 $
 * $State: Exp $ $Locker:  $
 *
 ...
 * add Xtree extender
 *
 * Revision 1.10  2011/08/17 13:21:16  thatr500
 * *** empty log message ***
 *
 * Revision 1.9  2011/08/17 13:15:27  thatr500
 * used 'fabs' instead of 'abs'
 *
 * Revision 1.8  2011/08/17 08:39:17  thatr500
 * used Manhattan distance computation instead of Euclidean
 *
 * Revision 1.7  2011/01/25 12:24:52  thatr500
 * supports EXINMA delivered as a DLL (Windows)
 *
 * Revision 1.6  2010/12/31 04:26:20  thatr500
 * *** empty log message ***
 *
 * Revision 1.5  2010/12/31 04:23:14  thatr500
 * Warnings on Unix are removed
 *
 * Revision 1.4  2010/12/31 03:41:51  thatr500
 * change INFINITY to XTREE_INFINITY to remove warning
 *
 * Revision 1.3  2010/12/27 10:57:50  thatr500
 * Remove warnings
 *
 * Revision 1.2  2010/12/01 19:20:02  thatr500
 * use EXSTMA to integerate Xtree
 *
 * Revision 1.1  2010/08/20 13:46:03  thtr1663
 * add xtree package
 *
...
double cal_distance(node_type *node, float *query, config_type fconfig) {
  int i;
  double distance;
  distance = 0.0;
  ...
  for (i = 0; i < fconfig.dim; i++)
	...
	distance += fabs(node->a[i] - query[i]);
...
void k_NN_NodeSearch(node_type *curr_node, float *query, NN_type *NN, int k, 
  ...
  if (curr_node->ptr[0]->attribute == LEAF && curr_node->object != nil) {
    ...
    for (i = 0; i < total; i++) {
      dist = cal_distance(curr_node->ptr[i], query, fconfig);
      if (dist < NN->dist)
...
void xtree_rangequery_result_update(NN_type *NN, double dist, node_type *node) {
  ...
  NN->pointer = node;
  /*Dec 1st : 
    + Since I rewrite (PICTUREFEATURES P _V2) then I have to bound value to P,
    _V2. P is a charstring whilst _V2 is a vector of number.
    
    + Now what is stored in node->object is a list of (ex. charstring)
    + So, instead of passing a node->object, there is an attempt to make a list
    ()    
  */
...
int xtree_rectangle_search(node_type *curr_node, float *query, NN_type *NN, 
  ...
  if (curr_node->attribute == LEAF && curr_node->object != nil) {
    ...
    distance = cal_distance(curr_node, query, fconfig);
    // normalize the value
    //if (distance > 0) {
    //  distance = 1/ distance;
    //}
