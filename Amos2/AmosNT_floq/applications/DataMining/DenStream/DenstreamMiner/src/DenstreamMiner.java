 ...
 * miro-clusters. At the same time, noise in the original data will be identified. Furthermore, the outlier micro-clusters have lower priority than potential 
 * core-micro-clusters to merge new data points. After that, bigger clusters will be formed by the miro-clusters by an algorithm similar to DBSCAN. 
 */
...
public class DenstreamMiner {

	public DenstreamMiner(){
    }
...
//			  	    }
		...
		try{
            ...
            //arraylists to store the purity of each micro-cluster
...
//            ArrayList<ArrayList<ArrayList<Double>>> oPurity=new ArrayList<ArrayList<ArrayList<Double>>>();
            ...
            //file to store the purity at times
//		    String outp="/Users/tangxuan/Master-thesis-CS/programming/work/data/real data/kdd99/purity_0.05_o.txt";
//		    File f=new File(outp);
//			FileOutputStream outPurity=new FileOutputStream(f);
		    
		    String stringLine;//get the lines read from file
		    double [] datapoint=new double[dim+2];//each point will be read into array, and the first element is the time stamp
...
//		  	    }
                ...
                int merge=0;//if the new point has been emerged: 0==no, 1==yes

                //System.out.println("*************************New datapoint is coming*******************************");
                //System.out.println("P-micro-cluster buffer is empty or not:"+pMicroBuff.isEmpty());
                ...
                //the mount of fading from current time to lastUpdateTime
                double fad=Math.pow(2,-1*lamda*(datapoint[0]-lastUpdateTime));
                
				//find the nearest potential micro-clusters and merge the new datapoint into them
	    		if(!pMicroBuff.isEmpty()){
			        ...
			        int [] minIndex=new int[overlapping];//index array of micro-clusters which have the min-distance with new data point
					...
					if(smallDisList.size()!=0){
					    ...
					    //System.out.println("P-micro-cluster Buffer: the smallest distance to one p-micro-cluster is:"+Math.sqrt(minDistance));
						//merge the new datapoint into the nearest p-micro-clusters
						for(int i=0;i<overlapping;i++){
							if(minDistance[i]<0) break;
							if(minDistance[i]<=epsilon*epsilon){
								merge=merge+1;
								//System.out.println("P-micro-cluster Buffer: merge the new point to one p-micro-cluster");
								...
								//update the purity of potential core-micro-clusters
//								int flag=0;
...
//								}
				...
				/*
				 *When a new point come in and can be assigned, try to emerge the data point into o-micro-cluster. However, if the o-micro-cluster buffer is empty
				 *or some condition can not be met, we should create a new o-micro-cluster.
		    	 */
				if(merge<overlapping){

					if(!oMicroBuff.isEmpty()){
						//System.out.println("O-micro-cluster Buffer:try to merge new point into one o-micro-cluster \n");
						
						double [] minDistance= new double[overlapping];//the min-distance between new data point and one micro-cluster
				        int [] minIndex=new int[overlapping];//index of one micro-cluster which has the min-distance with new data point
				        for(int i=0;i<overlapping;i++){
				        	minDistance[i]=-1;
				        	minIndex[i]=-1;
				        }
						...
						if(smallDisList.size()!=0){	
							for(int i=0;i<smallDisList.size();i++){
		                    	...
		                    	double updateWeight=oMicroBuff.get(smallDisList.get(i)).get(2).get(1)*fad;
		                    	for (int j=0;j<dim;j++){
		                    		double updateCF1=tmpCF1.get(j)*fad;
		                    		tmpDistance=tmpDistance +(updateCF1/updateWeight-datapoint[j+1])*(updateCF1/updateWeight-datapoint[j+1]);
		                    	}
							 ...
							 // merge the new data point into the micro-clusters
							 for(int i=0;i<overlapping;i++){
								if(merge==overlapping) break;
								if(minDistance[i]<0) break;
								if(minDistance[i]>epsilon*epsilon){
									if(merge==0){
										ArrayList<ArrayList<Double>> CF= new ArrayList<ArrayList<Double>>(3); //a feature vector for a cluster which is <CF1,CF2,WeightTime>
										...
										//WeightTime.add(0,lastUpdateTime);//last update time
										WeightTime.add(0,new Double(datapoint[0]));//created time
...
//										oPurity.add(tempV);
									...
									//update the puirty of micro-cluster
//										int flag=0;
...
//										}
							...
							//create a purity record for the new micro-cluster
//							ArrayList<Double> tmpV=new ArrayList<Double>();
//							ArrayList<ArrayList<Double>> tempV=new ArrayList<ArrayList<Double>>();
//							tmpV.add(datapoint[dim+1]);
//							tmpV.add(new Double(1));
		    	...
		    	if (datapoint[0]>=Tp*counterTp || datapoint[0]>=counterInterval*timeInterval){
		    		...
		    		if(!pMicroBuff.isEmpty()){
						for(int i=0;i<pMicroBuff.size();i++){
							...
							//update the last update time and weight
							//double lastUpdateTime=oMicroBuff.get(i).get(2).get(0);
							//tmpWeight.set(0,new Double(datapoint[0]));
		    		...
		    		//update find out the outlier micro-clusters with weight>=beta*u or weight<"specific value" in outlier micro-clusters buffer
		    		if(!oMicroBuff.isEmpty()){
						for(int i=0;i<oMicroBuff.size();i++){
		    		...
		    		//System.out.println("beta*u="+beta*u);
		        	//System.out.println("Before conversion: The size of conversion from o-micro-cluster to p-micro-cluster:"+oToPCluster.size());
		        	//System.out.println("Before conversion: The size of deletion in o-micro-clusters:"+delCluster);
...
//		    		}
		    		...
		    		//send the centers of potential core-micro-clusters and their cluster label to Amos II
		    	    try {
		    	    	//print the center of micro clusters into file
		    			for(int j=0;j<collectionOfClusters.size();j++){
