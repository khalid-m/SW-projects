...
;;; $Revision: 1.12 $ $Date: 2009/02/10 14:25:50 $
;;; $State: Exp $ $Locker:  $
;;;
;;; Description: Linear Road statistics update functions
;;; =============================================================
;;; $Log: visited.lsp,v $
;;; Revision 1.12  2009/02/10 14:25:50  zeitler
;;; binary LR regress
;;;
;;; Revision 1.11  2008/02/22 18:18:20  zeitler
;;; - More LR tests
;;; - correct sdiv
;;;
...
create function add_stat(Vector rep, Integer mn)->Boolean
  as foreign 'add-stat';
create function add_statbin(Iarray rep, Integer mn)->Boolean
  as foreign 'add-statbin';")

(defun add-statbin (fno rep mn);; MN: minute 
  (let ((vehicle (na-elt rep 2));; Vehicle ID
	(v (na-elt rep 3));; speed
	(x (na-elt rep 4));; xway
	(d (na-elt rep 6));; direction
	(s (na-elt rep 7));; Segment
