...
int main(int argc, char* argv[])
{
  ...
  if (argc < 2)
    {
      cout << "You have to provide an input data file name as a parameter." << endl;
      return 0;

    }

  // char* dataFile = argv[1];
  char* dataFile=(char*)malloc(sizeof(char*)*200);
  ...
  int ret = provider->Initialize(dataFile,100000, &mutex_lock);
      ...
      if( provider->PrepareData(provider) == SUCCESS )
	{
	  ...
	  for(;;)
	      ...
	      for(;;)
		{
		  ...
		  if ( nTuplesRead < nMaxTuples ) //< i originalet
		    {
