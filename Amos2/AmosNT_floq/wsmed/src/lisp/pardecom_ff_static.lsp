  ...
  (let ((i 1) par)
    (dolist (x (argsof 'and orgplan))
	  ...
	  (if  (and (or _parallelizable_ (= _no_of_rr_receive_ 0))(> (no-of-parfn fn orgplan) 1)(not (= (seventh x) (vector))))
	      (progn 
		(cond ((= _no_of_rr_receive_ 0)
		       (setq _wsl_ (list 0 0))
		       (setq _fanoutl_ (list ))))
		(setq _fanoutl_ (cons (list (1+ _no_of_rr_receive_) (nth _no_of_rr_receive_  _fanout_) (sixth x)) _fanoutl_));; initialize fanout detail per level
  ...
  (let* (sp  bsp rr_pos str)
	...
	(progn (setq  rr_pos _no_of_rr_receive_)
	       ...
	       (andify  `( (, _makebag_  , bsp ,@ (function-argvars bsp) BG) 
			   (, _ff-applyp_ ,  (listtoarray  _fanout_)  , rr_pos , sp BG , (length (function-resvars sp)) ,@ (function-resvars sp) )))))
