...
(defglobal _rr-receive_
 (osql "
create function rr_receive(Vector fanout, Integer rrpos, function fn, bag args, Integer width)
                                 -> object
...
(defun rr-receive (fno fanoutl  rrpos fn args width res)
  (let* (peerno ports sockets rd peers
		(thispeer -1) (np 0) (pp -1) fol)
	...
	(progn 
	  ...
	  (while (neq (length (amos-servers)) (1+ np)))));; wait until all processes are ready) 
