...
;;; $RCSfile: subplan_mixed.lsp,v $
;;; $Revision: 1.3 $ $Date: 2008/11/05 15:18:20 $
;;; $State: Exp $ $Locker:  $
;;;
;;; Description: Plan partitioner
;;; =============================================================
;;; $Log: subplan_mixed.lsp,v $
;;; Revision 1.3  2008/11/05 15:18:20  msabesan
;;; check for changes always in the subplan creation
;;;
;;; Revision 1.2  2008/10/27 07:33:12  msabesan
;;; *** empty log message ***
;;;
;;; Revision 1.1  2008/10/24 16:25:52  msabesan
...
(defun find-filter-arg (argl resl)
  "Find the filter arguments"
  (let ((found (list)))
    (dolist (x argl)
      (dolist (y resl) 
	(dolist (z lookupl)
	  (if (and (eq x (second z)) (eq y (first z)))
	      (setq found (adjoin(list y x) found))))
	))
    found))
...
(defun section-subplan (andl from to argl resl)
  ...
  (let* ((tr (predify-tbr andl))
   ...
   (setq spresl (replace-arg spresl));; To enable the replacements of parameters
   ...
   (setq middle (replace-pred middle))
	...
	(progn  (setq rrl (intersection fvmiddle (intersection fvfirst resl)))
		(if (and (not rrl)(intersection fvfirst resl)) 
		    (progn  (quote (setq spresl (replace-arg spresl)))
		...
		(if _dependent_
		    (progn
		      (setq ml (find-filter-arg  argl resl))
		      (dolist (x ml)
			(setq spresl (adjoin  (first x) spresl))
			(setq spargl (adjoin  (second x) spargl))
			(setq middle (append2 middle ( list `( , _=_  , (first x) , (second x))))))
		      ))
		)
      (progn 
	(if _dependent_
	  ...
	  (progn  
	    ...
	    (if templ;; renaming filter arguments and add additional predicates
		(progn
		  (rename-arg templ)
