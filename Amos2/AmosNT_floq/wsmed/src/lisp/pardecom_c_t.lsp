...
(defglobal _initnodes_ 10)
(defglobal _dmp_ "wsmed.dmp")
...
(defun no-of-parfn (fn orgplan)
  ...
  (let ((i 0)) 
    (dolist (x (argsof 'and orgplan))
	  ...
	  (progn 
	    (setq i (1+ i))
	    (if (eq _no_of_rr_receive_ 0) 
		(setq _wsl_ (adjoin (list (nth 3 x) (nth 4 x) (nth 5 x)) _wsl_))))))
    (quote 
    (if (eq _no_of_rr_receive_ 0) 
	(progn (setq _wsl_ (cdr (reverse _wsl_)))
	       (setq _fanout_ (arrange-fanouts  _wsl_ _maxout_ _maxpeers_)))))
    i))
  ...
  (let* (sp  bsp rr_pos str)
	...
	(progn (setq  rr_pos _no_of_rr_receive_)
	       ...
	       (andify  `( (, _makebag_  , bsp ,@ (function-argvars bsp) BG) 
			   (, _rr-receive_ , _dmp_  , _initnodes_ , sp BG , (length (function-resvars sp)) ,@ (function-resvars sp) )))))
...
(defun arrange-fanouts (wsl  maxout maxpeers)
"Arrange the fanouts to dynamically allocate nodes"
  (let ((i 0)(rest maxpeers) (premaxc 1) fanoutl maxc)
    (dolist (x wsl)
      (setq maxc (getmaxc (nth 0 x)(nth 1 x) (nth 2 x) ))
        (if (and ( < maxc maxout) (> (* maxc premaxc)  premaxc) (<= (* maxc premaxc) rest))
	  (progn (setq premaxc (* maxc premaxc))
		 (setq fanoutl (adjoin maxc fanoutl))
		 (setq rest (- rest premaxc)))
	(if (and (>= maxc maxout)(<= (* maxout premaxc) rest))
	    (progn (setq premaxc (* maxout premaxc))
		   (setq fanoutl (adjoin maxout fanoutl))
		   (setq rest (- rest premaxc)))
	  (if (> rest  premaxc)
	      (progn  (setq fanoutl (adjoin (/ rest (* premaxc 1.0)) fanoutl))
		      (return (reverse fanoutl)))
	    (return (reverse fanoutl))))))
    ...
    (cond (*skip-optimization* nil)
	  (t      
	   ...
	   ;;(break parallelize)
	    ...
	    (if (eq _amosid_ 'me)
		(if (> (length _fanout_) _no_of_rr_receive_) 
