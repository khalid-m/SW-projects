 ...
 * $Revision: 1.18 $ $Date: 2010/04/28 14:46:29 $
 * $State: Exp $ $Locker:  $
 *
 * Description: Utility functions for stand-alone SparQL parser
 * ===========================================================================
 * $Log: sparqlUtils.c,v $
 * Revision 1.18  2010/04/28 14:46:29  fred2431
 * Compile and regression test for SWARD under Linux
 *
 * Revision 1.17  2009/04/01 09:55:19  torer
 * Remove type RESOURCE
 *
 * Revision 1.16  2008/02/22 13:20:53  petrini
 ...
 *
 * Revision 1.15  2007/11/27 13:03:54  petrini
 * *** empty log message ***
 *
 * Revision 1.14  2007/11/26 18:55:51  torer
 * Fixed parser error handling problem
 *
 * Revision 1.13  2007/11/26 13:21:47  petrini
 * Error message gpErrMsg is set only if it is not set before. I.e. the first error is reported.
 *
 * Revision 1.12  2007/11/26 09:15:41  petrini
 * All memory in fn upvofuri allocated with sparql_new fn and deleted before next query is executed. this is the preferred way of allocating memory to strings in the SparQL parser.
 *
 * Revision 1.11  2007/11/24 14:35:34  torer
 * Bug that crashed SparQL-parser fixed!!
 *
 * Revision 1.10  2007/11/23 13:49:00  petrini
 * Fn parse_sparql now deletes allocated memory for every parsed query.
 *
 * Revision 1.9  2007/11/22 14:58:20  petrini
 ...
 *
 * Revision 1.8  2007/10/18 09:16:17  petrini
 ...
 *
 * Revision 1.6  2007/10/05 13:03:03  silvias
 * *** empty log message ***
 *
 * Revision 1.5  2007/07/27 17:13:21  torer
 * DLL initialization without modifying system source code
 *
 * Revision 1.4  2007/07/24 05:55:37  petrini
 * *** empty log message ***
 *
...
#define MAX_UPV_LEN             1024
#define MAX_ERR_LEN             1024
#define MAX_REG_LENGTH          128 
/*****************************************
				Structure
******************************************/
typedef struct{
...
/*Added by Johan Petrini 10/17/07*/
//int anyParserErrorID;
...
/* itoa is not an standard function so here it is*/
char *amos_itoa( char *str, int num, int base)
{
  sprintf( str, "%d", num );
  return str;
}

/*Get the UPV of a URI */
char* upvofuri(char *uri)
{
  dcl_connection(c);
  dcl_tuple(argl); /* Allocate tuple handle to hold argument lists */
  dcl_scan(s); /* s is a scan (stream) handle from results of Amos II queries and function calls */
  dcl_tuple(row); /* row is a tuple handle */
  dcl_oid(fn);

  a_connect(c,"",FALSE);
  a_newtuple(argl,1,FALSE); /* There is one argument */
  a_setf(fn,a_getfunction(c,"CHARSTRING.UPVOFURI->CHARSTRING",FALSE));
  a_setstringelem(argl,0,uri,FALSE); /* Bind string to first argument */
  ...
  {unwind_protect_begin; 
   ...
   if(!a_eos(s))
    { 
      gRDF_upv = sparql_new(MAX_UPV_LEN);
      a_getrow(s,row,FALSE); /* Move single scan element to row */
      a_getstringelem(row,0,gRDF_upv,MAX_UPV_LEN,FALSE);
    }   
   else 
    { 
      gRDF_upv = sparql_new(strlen("")+1);
      strcpy(gRDF_upv,"");
      
      if(gpErrMsg == NULL){
      gpErrMsg = sparql_new(MAX_ERR_LEN);
      strcpy(gpErrMsg,"Either no UPV associated to URI or no default UPV defined");
      }
    }  

   unwind_protect_catch; /*In all cases do*/
...
*/
void parse_sparql(a_callcontext cxt, a_tuple tpl)
{
  char *q;
  ...
  if (SparQLparse()==0)
      ...
      if (gpErrMsg != NULL)
	{
     ...
     a_error(parserErrorID, mkstring((gpErrMsg?gpErrMsg:"unknown reason")), 
               FALSE);
  }
}
...
/* Create a new literal
   ...
   if op==T_STR, 'type' is 
   a SparQL string;
   Modified by Johan Petrini 07-06-07. Removed calls to r().
*/
...
/************************************/
/*Modified by Johan Petrini 07-06-07. Removed calls to uri()*/
int _new_triple_(SparQLType s, SparQLType p, SparQLType o, int iOptional)
{
	...
	if (T_TYPE_MASK(o)==T_LTR)
	{
		...
		len = strlen(ss)+strlen(pp)+strlen(oo)+strlen(gRDF_parser)+
					strlen("(,)=<\"\",\"\">")+strlen(gRDF_src)+1;
		dst = sparql_new(len+1);

                /*Modified by Johan Petrini 10/17/07: Removed call to rdf_tr fn. 
                  Now the fn representing the RDF view is called instead.*/		
		/*subject: IRI or blank node; predicate: IRI or blank node; object: literal*/
		/*sprintf(dst, "%s(%s,%s)=<", gRDF_parser, gRDF_src, oo);*/

                /*If no FROM clause then use default UPV*/
                if(strcmp(gRDF_src,"\"\"") == 0){sprintf(dst, "%s(%s)=<", upvofuri(""), oo);} 
                else{sprintf(dst, "%s(%s)=<", upvofuri(gRDF_src), oo);}
		...
		/*sprintf(dst, "%s(%s)=<", gRDF_parser, gRDF_src);*/
                ...
                /*If no FROM clause then use default UPV*/
                if(strcmp(gRDF_src,"\"\"") == 0){sprintf(dst, "%s()=<", upvofuri(""));}
                else{sprintf(dst, "%s()=<", upvofuri(gRDF_src));}
...
/* Generates the from_clause
   in AmosQL. Modified by Johan Petrini 07-06-07 -> Uses Charstring instead of Resource.
*/
 ...
 * Modified by Johan Petrini 08-06-07. Removed vector constructor in result.
 */
char* output_query(void)
{
	...
	append_to_list_back(AmosQLLst, 
		mk_str("select"),mk_str(" "),gDistinct,gQueryVar,amos_decl_var(),T_LST_END);
	append_to_list_back(AmosQLLst, 
	...
	******************************************/
	...
	{
		char buffer[5];
		amos_itoa(gOBI[0].index,buffer,10);
		...
		{
			amos_itoa(gOBI[i].index,buffer,10);
...
void areg(a_callcontext cxt, a_tuple params)
{
  char areg[MAX_REG_LENGTH];
  char sreg[MAX_REG_LENGTH];
  char* p; 
  int csize;
  int size;
  int ac;  
  ...
  csize = sizeof(char);
  p = getstring(a_getobjectelem(params, 0, FALSE));
  a_getstringelem(params,0,sreg,sizeof(sreg), FALSE); //checking type
  size = strlen(sreg);    
  ...
  areg[ac]='\0';
  a_setstringelem(params,1,areg,FALSE);
...
{
  a_extfunction("parse_sparql", parse_sparql);
  a_extfunction("areg", areg);
