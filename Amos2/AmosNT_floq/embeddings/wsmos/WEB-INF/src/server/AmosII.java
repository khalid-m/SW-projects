...
//	    argsName = row.getStringElem(3);
	    ...
	    if(argsType.equals(""))
		fullName = functionName+"->"+resultAtt.replace(",", ".");
	    else
		fullName = argsType+"."+functionName+"->"+resultAtt.replace(",", ".");
...
//		this.stopAfter = stopAfter;	
		...
		try{
			theScanner = theConnection.callFunction("CHARSTRING.CHARSTRING.GetFunInfo->CHARSTRING.CHARSTRING", arg);
			Tuple row = theScanner.getRow();
			resultAtt = row.getStringElem(0);
			resName = row.getStringElem(1);
		}catch(AmosException e){
