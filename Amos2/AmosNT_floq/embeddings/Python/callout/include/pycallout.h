 ...
 * $Revision: 1.4 $ $Date: 2011/05/02 18:05:57 $
 * $State: Exp $ $Locker:  $
 *
...
#define PYTHON_FOREIGN_SYNTAX " The syntax is 'py:binding_pattern:[module.]function'."
...
int pycallout_parse_ff(char *, pycallout_ff_t *);     /* Parse python foreign function string. */
int pycallout_bind(char *);                           /* Register a python foreign function with pycallout_call. */
oidtype pycallout_reload_foreign(a_callcontext);      /* Reload previously registred foreign function. */ 
oidtype pycallout_call(a_callcontext);                /* Calls the python foreign functon that is associated with the currently called amos function. */
