...
{ 
    if(SOAPClient._delay == undefined){ //no user input timeout value
        SOAPClient._delay = 50000;//setup time out by default
    }
...
SOAPClient._getURL = function (async,errorcallback){
	...
	xmlDoc.load("http://localhost/CourseManager/WEB-INF/web.xml");
        }else if (window.XMLHttpRequest){
            xmlDoc = SOAPClient._getLocalXml(async,"http://localhost/CourseManager/WEB-INF/web.xml",errorcallback).responseXML;
        }
        return xmlDoc;
    }
    catch(e){
...
SOAPClient._getSoapResponse = function(method, async, callback, errorcallback, req)
{
    ...
    //var nd = SOAPClient._getElementsByTagName(req.responseXML, "tns:results", async, errorcallback); //AmosSoapServer soap response type
      var nd = SOAPClient._getElementsByTagName(req.responseXML, req.responseXML.firstChild.firstChild.firstChild.nodeName, async, errorcallback); //AmosSoapServer soap response type
    if (nd.length == 0){
