...
void error_reason(char *in)
{
	if (gpErrMsg == NULL)
	{
		gpErrMsg = sparql_new(strlen("set LastErrorMsg()=\"Not support: '%s'\";")
			+strlen(in)+1);
		sprintf(gpErrMsg, "set LastErrorMsg()=\"Not support: '%s'\";", in);
	}
}
/*****************************************
...
void sparql_query(a_callcontext cxt, a_tuple tpl)
{
	...
	if (gpErrMsg != NULL)
	{
		/* error in processing the SPARQL statement */
		dcl_scan(s);
		a_execute(sparql_amos_c, s, gpErrMsg, FALSE);
		free_scan(s);
	}
}
...
/************************************/	
/* Parser SPARQL statement
   Statement is stored in a file
*/
void sparql_from_file(a_callcontext cxt, a_tuple tpl)
{
	char *q=NULL, *fn=NULL;
	FILE *fin=NULL;
	long fsize=0;
	...
	/*SPARQL query statement file*/
	a_getstringelem(tpl, 2, gpTmp1, MAX_INPUT, FALSE);
...
void sparql_set_amos_connection(void* c)
{
	...
	a_extfunction("sparql_from_file", sparql_from_file);
