...
public class User extends Oid {
	...
	public static boolean authenticate(Connection db, User theUser,
			String password, String userField, String passField, Hashtable err) {
		boolean isValid = true;
		try {
			if (theUser != null) {
				String pass = theUser.getPassword();
				if (pass == null) {
					//err.put(passField,"This user has no password. Contact your administrator.");
					isValid = false;
	...
	public static User getUser(Connection c, String name) throws AmosException {
		Scan theScan;
		theScan = c.callFunction("CHARSTRING.USERNAMED->USER", new Tuple(name));
		if (!theScan.eos()) {
	...
	private String getPassword() throws AmosException {
		Scan theScan;
		theScan = this.getConnection().callFunction(
				"USER.PASSWORD->CHARSTRING", this);
	...
	public static User getUserElem(int i, Connection c) throws AmosException {
	...
	public StringScan getLastname() throws AmosException {
		...
		return new StringScan(con.callFunction("USER.LASTNAME->CHARSTRING",
				this), con);
	}
	...
	public StringScan getEmail() throws AmosException {
		...
		return new StringScan(con.callFunction("USER.EMAIL->CHARSTRING", this),
				con);
	}
	...
	public StringScan getFirstname() throws AmosException {
		...
		return new StringScan(con.callFunction("USER.FIRSTNAME->CHARSTRING",
				this), con);
	}
