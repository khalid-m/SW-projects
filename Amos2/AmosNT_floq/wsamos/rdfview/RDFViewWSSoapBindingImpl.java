...
public class RDFViewWSSoapBindingImpl implements rdfview.RDFViewWS{
   
private Connection theConnection;
    private String dbServer="RDFVIEWER";
    private boolean initialized = false;
    private java.util.Map opRegistry = new java.util.HashMap();
    
    public RDFViewWSSoapBindingImpl(){
	
	if (!initialized)
	{

	   String dmp= "amos2.dmp";
	    try {
		Connection.initializeAmos(dmp);  
	...
	Scan theScan=theConnection.execute("select WS_operations_descr();");
	while (!theScan.eos()){
