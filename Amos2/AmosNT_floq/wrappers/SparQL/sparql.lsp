...
(/putobject _sparql_ 'initializer 'sparql-initialize)
(/putobject _sparql_ 'finalizer 'sparql-finalize) 

(osql "create function sparql_query(charstring query, charstring address)->vector as foreign 'JAVA:sparql_interface.SparQLQuery/sparqlQuery';")
...
(defun sparql (fno name address ds)
  ...
  (let* ((fn-lisp-name (concat (lispify-name name) "-CC"))
    ...
    (let ((cc-fno (createfunction fn-amos-name
      ...
      (/putobject cc-fno 'defaultabsorbent 'sparql-translate-core-cluster)
      ...
      (create-mapped-type (mksymbol name)
			  ...
			  '((charstring s) (charstring p) (charstring o))
			  cc-fno)
      (getfunction 'create_capability (list ds cc-fno "fff" "sparql-translate-core-cluster"))
...
(defun sparql-initialize (ds env)
  (make-sparql-query :bvar-count 0 :bvar-db (make-hash-table)))
...
(defun sparql-finalize (ds env acc)
  (let* ((query-string (create-query-string acc))
	 (query-function (create-amos-query-function ds query-string acc)))
    ;(print query-string)
    (when (sq-outvars acc)
      (declarecosts query-function '*any* 'sparql_cost)
      `((,_apply_pred_ ,query-function ,@(sq-invars acc) ,@(sq-outvars acc))))))
...
(osql "create_capability(typenamed('sparql'),functionnamed('charstring.charstring.like->boolean'),'bb','sparql-translate-like');")
...
(osql "create_capability(typenamed('sparql'),functionnamed('object.object.<->boolean'),'bb','sparql-translate-comparison');")
(osql "create_capability(typenamed('sparql'),functionnamed('object.object.<=->boolean'),'bb','sparql-translate-comparison');")
(osql "create_capability(typenamed('sparql'),functionnamed('object.object.>->boolean'),'bb','sparql-translate-comparison');")
(osql "create_capability(typenamed('sparql'),functionnamed('object.object.>=->boolean'),'bb','sparql-translate-comparison');")
