...
;;; HP-trie-based version, avgv is implemented in C
;;; B-tree calls replaced with HP trie (Judy) calls
;;;
...
(defglobal _cars_;; Number of vehicles 
  (vector (make-trie) (make-trie)))

(defglobal _stat_;; To compuite avg speeed in a segment last five minutes
  (vector (make-trie) (make-trie) (make-trie) 
	  (make-trie) (make-trie) (make-trie)))
...
(defun init-stat (fno mn)
  (let  (
    ...
    (free-trie (AREF _cars_ i));;free the tries that were created before
    (free-trie (AREF _stat_ j))
    (seta _cars_ i (make-trie));create new tries
    (seta _stat_ j (make-trie))
    )
  )
...
(defun add-stat (fno rep mn);; MN: minute 
  (let ((vehicle (elt rep 2));; Vehicle ID
    ...
    (let* ((w (trie-key (vector s x d vehicle))) 
	   (tmp (get-trie w stat)))
      (put-trie w (elt _cars_ (mod mn 2)) t) 
      ;; Record different cars this minute
      (cond (tmp (rplacd tmp (/ (+ v (* (car tmp) (cdr tmp)))
				(+ 1.0 (car tmp))))
		 (rplaca tmp (1+ (car tmp))))
	    (t (put-trie w stat (cons 1 (+ 0.0 v))))))));; add 0.0 so that v is always a floating point number
...
(defun count-visited (fno s x d mn r)
  (let ((count (count-trie (elt _cars_ (mod mn 2))
...
(defun avg-v (fno s x d mn r)
  ( osql-result s x d mn (avg-v-c (vector s x d '*) mn  _stat_ ))
)



(osql "
create function avgvo(Integer s, Integer x, Integer d, Integer mn)->Number
  as foreign 'avg-v-o';")

(defun avg-v-o (fno s x d mn r)
  (let  ((v 0) 
	(n 0)
	(low (trie-low-range (vector s x d '*)))
	(high (trie-high-range (vector s x d '*)))
        (except (mod (+ 1 mn) 6)))
    (maparray
     _stat_ (f/l (ct i)
		 (cond ((eq i except))
...
(defun pre-lav (fno s x d mn r)
  (let*  (
    ...
    (map-trie (elt _stat_ (mod mn 6)) low high  
	       (f/l (key cell)
		    (setq v (+ v (cdr cell)))
		    (1++ n)))
