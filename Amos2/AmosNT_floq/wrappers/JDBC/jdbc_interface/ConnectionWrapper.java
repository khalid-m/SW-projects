...
public class ConnectionWrapper {
    ...
    public int maxBinarySize = 4096; //bytes //AA
    public PreparedStatement bps = null; //prepared statement for batch execution //AA
    public int bpsCnt, bpsLimit = 1024; //AA
    ...
    public java.sql.Connection getConnection() { //AA
	return conn;
    }
	...
	ResultSetWrapper rsw = new ResultSetWrapper(ps, false);
	rsw.maxBinarySize = maxBinarySize; //AA
	return rsw;
    }
    ...
    public void startBatchQuery(String query) //AA
	throws SQLException, AmosException {
	bps = producePreparedStatement(query); 
  	bpsCnt = 0;
    }

    public void addToBatch(Tuple args) //AA
	throws SQLException, AmosException {
	if (bps == null) 
	    throw new AmosException("No batch is active");	
	fillInPreparedStatement(bps, args);          
	if (bpsCnt >= bpsLimit) {
	    executeBatch(false);
	    bpsCnt = 0;
	}
    ...
    public ResultSetWrapper executeBatch(boolean doClear) //AA
	throws SQLException, AmosException {  
	if (bps == null) 
	    throw new AmosException("No batch is active");
      	ResultSetWrapper rsw = new ResultSetWrapper(bps, true);    
	if (doClear) {
	    bps.close();
	    bps = null;
	...
	rsw.maxBinarySize = maxBinarySize; 
	return rsw;
    }
	...
	for (int i = 0; i < args.getArity(); i++) {
	    ...
	    else if (args.isBinary(i)) { //AA
		pstmt.setBytes(i+1, args.getBinaryElem(i));
	    }		
