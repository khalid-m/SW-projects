 ...
 * $Revision: 1.20 $ $Date: 2013/10/18 16:05:11 $
 * $State: Exp $ $Locker:  $
 *
...
/*
    public static Oid inFn; //AA

    public JDBCInterface() { //AA
	try {
	    Connection conn = new Connection(""); //Amos connection
	    inFn = conn.getFunction("BAG.TUPLES->VECTOR"); //TODO: variant "BAG.IN->OBJECT"
	    System.out.println("JDBCInterface initialized"); //DEBUG
	} catch (AmosException e) { System.out.println("WARNING: Failed to obtain 'in' function handle"); }
    }
*/    
  ...
  public void disconnect(CallContext cxt, Tuple tpl) 
    ...
    public void setMaxBinarySize(CallContext cxt, Tuple tpl) //AA
        throws AmosException, SQLException, java.io.IOException {
	
	ConnectionWrapper cw = ConnectionWrapper.getConnectionWrapper(tpl.getOidElem(0));
	cw.maxBinarySize = tpl.getIntElem(1);
    }
   ...
   */
    ...
    public void setAutoCommit(CallContext cxt, Tuple tpl) //AA
	throws AmosException, SQLException, java.io.IOException {
	
	Oid datasource = tpl.getOidElem(0);
	int v = tpl.getIntElem(1);
	ConnectionWrapper cw = ConnectionWrapper.getConnectionWrapper(datasource);
	cw.getConnection().setAutoCommit(v==1);
    }

    public void commit(CallContext cxt, Tuple tpl) //AA
	throws AmosException, SQLException, java.io.IOException {
    ...
    public void executeParametrizedQueryBatch(CallContext cxt, Tuple tpl) //AA
	throws AmosException, SQLException, java.io.IOException  {
      
	Oid datasource = tpl.getOidElem(0);
	String  query = tpl.getStringElem(1);
	//	Oid argBag = tpl.getOidElem(2);
	Tuple  arguments = new Tuple(),
	    inArgs = new Tuple(tpl.getOidElem(2));
	ConnectionWrapper cw = ConnectionWrapper.getConnectionWrapper(datasource);
        ResultSetWrapper rsw;

	Connection conn = new Connection(""); //Amos connection
        Oid inFn = conn.getFunction("BAG.TUPLES->VECTOR"); //TODO: variant "BAG.IN->OBJECT" 
	Scan argScan = conn.callFunction(inFn, inArgs);

	cw.startBatchQuery(query);
	while (!argScan.eos()) {
	    argScan.getRow(arguments);
	    cw.addToBatch(arguments.getSeqElem(0).getSeqElem(0));
	    argScan.nextRow();
	}
	argScan.close();
	argScan = null;

	rsw = cw.executeBatch(true);
	while (rsw.next()) 
	    {
...
/**
  ...
  public void getIndexInfo(CallContext cxt, Tuple tpl)
    ...
    boolean unique      = false; /*TODO Should be picked from tpl at 4*/
    boolean approximate = true;  /*TODO Should be picked from tpl at 5*/
    ConnectionWrapper cw = ConnectionWrapper.getConnectionWrapper(datasource);
    DatabaseMetaData dbmd = cw.getMetaData();
    ResultSet rs = 
      dbmd.getIndexInfo(catalog, schema, table, unique, approximate);
    
    while(rs.next()) {
      switch (rs.getShort("TYPE")) {
       case DatabaseMetaData.tableIndexStatistic:
	tpl.setElem(6,"tableStatistic");
	break;
      case DatabaseMetaData.tableIndexClustered:
       tpl.setElem(6, "tableIndexClustered");
       break;
      case DatabaseMetaData.tableIndexHashed:
       tpl.setElem(6, "tableIndexHashed");
       break;
      case DatabaseMetaData.tableIndexOther:
       tpl.setElem(6, "tableIndexOther");
       break;      
      }
      /*Table statistic does not have 'index name' and 'column name'*/
      if (rs.getShort("TYPE") != DatabaseMetaData.tableIndexStatistic) {
	  tpl.setElem(7, rs.getString("INDEX_NAME"));
	  tpl.setElem(8, rs.getString("COLUMN_NAME"));
          tpl.setElem(9, rs.getShort("ORDINAL_POSITION"));  
      }
      tpl.setElem(10,rs.getInt("CARDINALITY"));
      cxt.emit(tpl);
    }
    rs.close();
  }
