 ...
 * $Revision: 1.33 $ $Date: 2013/07/28 06:24:09 $
 * $State: Exp $ $Locker:  $
 *
 * Description: A decorator approach to return rows from a table as tuples.
 * ===========================================================================
 * $Log: ResultSetWrapper.java,v $
 * Revision 1.33  2013/07/28 06:24:09  andan342
 * Mapping BINARY SQL type to Amos BINARY
 *
 * Revision 1.32  2013/03/26 16:33:59  andan342
 * Implemented sqlbatch() function (timely garbage collection is still an issue!)
 *
 * Revision 1.31  2013/03/22 16:04:34  andan342
 * Changed the way ByteStream:s are processed - compatibility fix for MS SQL Server JDBC Driver
 *
 * Revision 1.30  2013/02/24 20:12:03  andan342
 * Converting VARBINARY to BINARY, NVARCHAR to CHARSTRING
 *
 * Revision 1.29  2013/02/22 23:13:13  andan342
 * Added support for JDBC4, returning -1 as update count for DDL statements
 *
 * Revision 1.28  2012/10/29 22:21:10  andan342
 * Reading BLOBs as BINARY and writing BINARY into BLOBs within JDBC interface
 *
 * Revision 1.27  2012/09/06 11:42:33  andan342
 * Now correctly handling empty strings in the result set
 *
 * Revision 1.26  2012/09/03 12:25:40  andan342
 * Fixed bug when processing queries involving heterogenous CONCAT() in SELECT list
 *
...
public class ResultSetWrapper {
    ...
    private static final int BINARY =8; //AA
	...
	Integer timeo = new Integer(TIME);
        Integer binaryo = new Integer(BINARY); //AA
	typeMap.put(new Integer(java.sql.Types.BIGINT),   integero);
	typeMap.put(new Integer(java.sql.Types.BINARY),   binaryo);
	...
	typeMap.put(new Integer(java.sql.Types.NVARCHAR),  stringo); //AA
	typeMap.put(new Integer(java.sql.Types.VARBINARY),  binaryo); //AA
	typeMap.put(new Integer(java.sql.Types.LONGVARCHAR),  stringo);
        typeMap.put(new Integer(java.sql.Types.BLOB),  binaryo); //AA: handle binary arrays similarly to strings
        typeMap.put(new Integer(java.sql.Types.LONGVARBINARY),  binaryo); //AA: even though "BLOB" is returned by ResultSetMetaData.getColumnTypeName for MySQL BLOB fields,
                                                                          // getColumnType returns -4, which corresponds to LONGVARBINARY, according to
                                                                          // http://docs.oracle.com/javase/1.4.2/docs/api/constant-values.html#java.sql.Types.BLOB

        
    }

    public int maxBinarySize; //bytes, set in ConnectionWrapper.executeQuery //AA
     ...
     */  
    public ResultSetWrapper(PreparedStatement theps, Boolean isBatch) throws SQLException {
        ps = theps;
	//	System.out.println("Constructing ResultSetWrapper, ps = "+ps); //DEBUG
	Boolean execRes;
	if (isBatch) {
	    ps.executeBatch(); //TODO: handle update count
	    execRes = false; 
	} else  execRes = ps.execute();

	if(execRes) 
	    {   int i, cc;
	    //	    System.out.println("Executed, rs="+rs); //DEBUG
	    rs = ps.getResultSet();
	    rsmd = rs.getMetaData();
		...
		//		System.out.println("Executed(false), RowsAffected="+ps.getUpdateCount()); //DEBUG
		rowsAffected = ps.getUpdateCount() ;
		if (rowsAffected < 0) rowsAffected = 0; //AA
		isOpen = true;
            }
    }
    /**
	...
	if (!isOpen) throw new AmosException("Result set wrapper is closed.");
		...
		row = new Tuple(width);
		    ...
		    {
			//System.out.println("Column "+i+" type "+rsmd.getColumnType(i+1)+" typename "+rsmd.getColumnTypeName(i+1)); //DEBUG //AA
			//System.out.println("typeMap(-4)="+(Integer)typeMap.get(new Integer(-4))+", BLOB="+java.sql.Types.BLOB); //DEBUG //AA
	...
	for (int i=0; i<width; i++) 
		...
		switch (datatypes[i]) 
			...
			break;
                    case BINARY: //AA
			{
			    java.io.InputStream fin = rs.getBinaryStream(i+1);			    
			    int ba, size = 0;			    
			    byte [] buff = new byte[maxBinarySize];
			    do {
				ba = fin.available();
				if (ba > 0) {
				    ba = fin.read(buff, size, ba);
				    size += ba;
				}
			    } while (ba > 0);
			    
			    //System.out.println("Column "+i+" BINARY, size="+size); //DEBUG

			    row.setBinaryElem(i, buff, size);
			    if (size > columnSize[i]) columnSize[i] = size;
			}
			break;
		    case STRING: 
			{
				...
				for(;;)
					...
					// if(size==-1) break; // end of stream //removed by AA
					if(first)
					    {
						first = false;
						if (size==-1) row.setElem(i,""); //AA
						else row.setElem(i, buff, size);
					    }
					else if (size==-1) break; //AA
	...
	//	System.out.println("next() called with rs=" + rs); //DEBUG
