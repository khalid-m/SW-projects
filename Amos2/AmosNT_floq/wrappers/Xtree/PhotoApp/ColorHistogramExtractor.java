...
public class ColorHistogramExtractor {
    ...
    public void extract(CallContext cxt, Tuple tpl) throws AmosException {
	try {
	    
	    m_filename = tpl.getStringElem(0);
	    m_histogram = new int[m_bins];
	    ...
	    // ----------------------------------------------------
	    // Fill in a tuple and emit it
	    // ----------------------------------------------------
	    	    
	    Tuple tmpTuple;
	    tmpTuple = new Tuple(m_bins);
	        
	    for(int i = 0; i < m_bins; i++) {
		tmpTuple.setElem(i, m_histogram[i]);
	    }
	    tpl.setElem(1, tmpTuple);
	    cxt.emit(tpl);
