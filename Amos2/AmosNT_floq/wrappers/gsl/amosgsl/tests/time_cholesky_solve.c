...
#define CHOLESKY_DATA_FILENAME "cholsolve.dat"
  ...
  do
    {
      ...
      agsl_linalg_cholesky_solve_alpha_ab (env, mkreal (ALPHA1),A, C);
...
cpu_amosql_cholesky ()
{
  ...
  sprintf (query, "select cholsolve(chol(eye(%d)), rand(%d));", size, size);
