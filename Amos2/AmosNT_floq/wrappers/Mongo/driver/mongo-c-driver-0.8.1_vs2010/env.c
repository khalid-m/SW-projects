...
/************************************************************************************/
    ...
    if ( ( conn->sock = socket( AF_INET, SOCK_STREAM, 0 ) ) == INVALID_SOCKET ) {
		...
		printf("Got from socket fn %d what error %d \n", INVALID_SOCKET, WSAGetLastError());		
        return MONGO_ERROR;
    }
