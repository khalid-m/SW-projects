 ...
 * $Revision: 1.13 $ $Date: 2014/01/20 15:52:11 $
 * $State: Exp $ $Locker:  $
 *
...
* Fn Sign     : create function nmongo_connect()-> Integer conn_no            *
*               as foreign 'mongo_connect';                                   *
******************************************************************************/
oidtype mongo_connectFB(a_callcontext cxt)
{  
  oidtype host;
  char *hostname;
...
*                               Untill  MAXCONN reached,                      *
* Returns     : Connection number as Ingeger                                  *
* Fn Sign     : create function mongo_disConnect()-> Boolean status           *
*               as foreign 'mongo_disConnectBF';                              *
******************************************************************************/
oidtype mongo_disConnectBF(a_callcontext cxt)
{  
  int conn_no;
  dcloid(conn_no_oid);
  conn_no_oid=a_arg(cxt,1);
  IntoInteger(conn_no_oid,conn_no,cxt->env); /* Unbox 1nd arg: conn_no */  
  if(mongoConnStatus(conn_no) != MONGO_OK)
  {
    return nil;
  }
  mongo_destroy(&conn[conn_no].mongo_conn);
...
******************************************************************************/
oidtype mongo_queryBBBBF(a_callcontext cxt)
{ 
  int conn_no;
  char mongoNameSpace[MAX_MONGO_NS];
  char *mongoDBName;
  ...
  while( mongo_cursor_next( cursor ) == MONGO_OK )
    {
      ...
      r = bson_data_to_record(cxt->env, (&cursor->current)->data);        
      a_bind(cxt,5,r);  //Bind the result value @parm 5
      a_result(cxt); /* Emit 1st solution */                                
    }
...
* Fn Sign     : create function mongo_add_batch(Integer  conn_no, Charstring  *
                ...
                Vector oid)-> Boolean status as foreign 'put_batch';          *
* WARNING     : Current Implementation can only handel 30,000 (approxmately ) *
                ...
                vector of records                                             *
******************************************************************************/
oidtype mongo_add_batchBBBBBF(a_callcontext cxt)
{
  int conn_no;
  char mongoNameSpace[MAX_MONGO_NS];  
  char *mongoDBName;
  ...
  IntoString(coll_oid,mongoCollName,cxt->env);/*Unbox 3rd : CollName*/   
  sprintf_s(mongoNameSpace,MAX_MONGO_NS,"%s.%s",mongoDBName,mongoCollName);  
  write_concern_oid = a_arg(cxt,4);
  IntoInteger(write_concern_oid,
	      write_concern_val,cxt->env); /* Unbox 4th arg: write_concern*/     
  vector_record = a_arg(cxt, 5); // Bind x to 5th argument  the vector record
  ...
  conn[conn_no].isInsert=0; /* As this is batch insert not single insert*/
  for(i=0;i<dim;i++)
    {       
...
/******************************************************************************
* function:     mongo_createIndex()                                           *
* Arguments :   int conn_no, string database, string collection,              *
                string index name, Record oid                                 *
* Descriptions: This  function creates a inedex based on the record(BSON)     *
                matches with the given Record (BSON)                          *
* Returns     : TRUE as Boolean if successfull, otherwise nil                 *
* Fn Sign     : mongo_createIndex(Integer  conn_no, Charstring  database,     *
                Charstring collection, Charstring indexName, Record oid) ->   *
                Boolean status                                                *
                as foreign 'mongo_createIndex-----+';                                   *
******************************************************************************/
oidtype mongo_createIndexBBBBBF(a_callcontext cxt)
{
  int conn_no;
  char mongoNameSpace[MAX_MONGO_NS];
  char *mongoDBName;
  ...
  IntoString(coll_oid,mongoCollName,cxt->env);/*Unbox 3rd : CollName*/   
  sprintf_s(mongoNameSpace,MAX_MONGO_NS,"%s.%s",mongoDBName,mongoCollName);   
  index_oid=a_arg(cxt,4); /*Unbox 4th : indexName*/   
  IntoString(index_oid,mongoIndexName,cxt->env);/*Unbox 3rd : index Name*/     
  rec = a_arg(cxt, 5); // Bind 5th argument to record 
  OfType(rec, recordtype, cxt->env);  
  if( mongoConnStatus(conn_no) != MONGO_OK ){//check for established conn
    if( mongoConn(conn_cout) != MONGO_OK ) { // othewise create connection
      printf("Connection Cannot be created\n"); 
  ...
  record_to_bson(cxt->env, &rec,b, conn_no);
  if( mongo_create_index( &conn[conn_no].mongo_conn, mongoNameSpace, b, 
    mongoIndexName, 0, 0, cmd)!= MONGO_OK ) {  
    //bson_print(cmd);          
    return nil; /* Always return nil */
  }  
  //bson_print(cmd);   
...
/******************************************************************************
* function:     mongo_run_cmd()                                               *
* Arguments :   int conn_no, string database, string collection, Record cmd   *
* Descriptions: This  function sends command to Mongodb Server based on the   *
                cmd record(BSON)                                              *
                matches with the given Record (BSON)                          *
* Returns     : TRUE as Boolean if successfull, otherwise nil                 *
* Fn Sign     : mongo_run_cmd(Integer  conn_no, Charstring  database,         *
                Record cmd) -> Record status                                  *
                as foreign 'mongo_run_cmd----+';                              *
******************************************************************************/
oidtype mongo_run_cmdBBBF(a_callcontext cxt)
{
  int conn_no;  
  char *mongoDBName;  
  bson bson_cmd[1], cmd[1];
  oidtype r;
  dcloid(rec);  
  dcloid(conn_no_oid);
  dcloid(db_oid);  
  ...
  db_oid=a_arg(cxt,2);
  IntoString(db_oid,mongoDBName,cxt->env);/*Unbox 2nd : DBName*/   
  cmd_oid = a_arg(cxt, 3); // Bind 3rd argument cmd to record 
  OfType(cmd_oid, recordtype, cxt->env);  
  if( mongoConnStatus(conn_no) != MONGO_OK ){//check for established conn
    if( mongoConn(conn_cout) != MONGO_OK ) { // othewise create connection
      printf("Connection Cannot be created\n"); 
      return nil; 
  ...
  record_to_bson(cxt->env, &cmd_oid,bson_cmd, conn_no);
  if(mongo_run_command( &conn[conn_no].mongo_conn, mongoDBName, bson_cmd, cmd )  
    != MONGO_OK ) {  
    //bson_print(cmd);          
    return nil; /* Always return nil */
  }  
  //bson_print(cmd);    
	r = bson_data_to_record(cxt->env, (cmd)->data);
  a_bind(cxt,4,r);  //Bind the result value @parm 5      
     ...
     */
{
  ...
  a_extimpl("mongo_connect+-",mongo_connectFB);  
  a_extimpl("mongo_disConnect-+",mongo_disConnectBF);
  a_extimpl("mongo_query----+",mongo_queryBBBBF);
  a_extimpl("mongo_add----+",mongo_addBBBBF);
  a_extimpl("mongo_add_batch-----+",mongo_add_batchBBBBBF);     
  a_extimpl("mongo_del----+",mongo_delBBBBF);  
  a_extimpl("mongo_createIndex-----+",mongo_createIndexBBBBBF);   
  a_extimpl("mongo_run_cmd---+",mongo_run_cmdBBBF);
